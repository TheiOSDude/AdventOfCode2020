...
struct Password {
    ...
    var isValid: Bool {
        ...
        let charAtInitialIndex = String(Array(password)[minimum - 1])
        let charAtEnd = String(Array(password)[maximum - 1])
        let initalMatch = charAtInitialIndex == requiredChar
        let endMatch = charAtEnd == requiredChar
        if initalMatch && endMatch {
            return false
        }
        return (initalMatch) || (endMatch)
//
    }
}
